...
void select_non_empty_entries(u64 data_vocab_size, double const * const restrict r,
...
void SDDMM(const u64 data_vocab_size, const u64 v_r, const u64 num_docs,
		double const * const restrict K_transpose,
		double const * const restrict u, double *w_csr) {
#pragma omp parallel for
	for (u64 i = 0; i < data_vocab_size; i++) {
		...
		for (u64 e = c_csr.row_ptr[i]; e < c_csr.row_ptr[i + 1]; e++) {
			...
			// #pragma vector
			for (u64 k = 0; k < v_r; k++) {
				sum += (K_transpose[i * v_r + k] * u[k * num_docs + j]);
			}
    ...
    // do it naively
    // w_csr = (K.T @ u)
    double * w_dense = new double [data_vocab_size*num_docs];
    // call dgemm or do your own dgemm
...
void SDDMM2(const u64 data_vocab_size, const u64 v_r, const u64 num_docs,
		double const * const restrict K_transpose,
		double const * const restrict u, double *w_csr, double * w_dense) {
    ...
    for (u64 i = 0; i < data_vocab_size; i++) {
		...
		for (u64 e = c_csr.row_ptr[i]; e < c_csr.row_ptr[i + 1]; e++) {
			u64 j = c_csr.col_inds[e];
            w_csr[e] = ((c_csr.vals[e] / w_dense[i*num_docs+j]));
        }
    }
}
...
void sinkhorn_wmd(const double *r, const double *vecs, double *WMD,
	...
	double *r_sel;
	posix_memalign((void**) &r_sel, ALN, (data_vocab_size) * sizeof(double));
	assert(r_sel != NULL);

	double *v_sel;
	posix_memalign((void**) &v_sel, ALN,
			(data_vocab_size * word2vec_word_embedding_size) * sizeof(double));
	assert(v_sel != NULL);
...
#endif
	double *M;
	posix_memalign((void**) &M, ALN, (v_r * data_vocab_size) * sizeof(double));
	assert(M != NULL);

	//# K=exp(-lambda * M)
	//  K = np.exp(- M * lamb)
	double *K;
	posix_memalign((void**) &K, ALN, (v_r * data_vocab_size) * sizeof(double));
	assert(K != NULL);

	double *K_transpose;
	posix_memalign((void**) &K_transpose, ALN,
			(v_r * data_vocab_size) * sizeof(double));
	assert(K_transpose != NULL);

	double *x;
	posix_memalign((void**) &x, ALN, (v_r * num_docs) * sizeof(double));
	assert(x != NULL);

	// u=1/x
	double *u;
	posix_memalign((void**) &u, ALN, (v_r * num_docs) * sizeof(double));
	assert(u != NULL);

	double *K_over_r;
	posix_memalign((void**) &K_over_r, ALN,
			(data_vocab_size * v_r) * sizeof(double));
	assert(K_over_r != NULL);

	double *w_csr;
	posix_memalign((void**) &w_csr, ALN, (c_csr.nnz) * sizeof(double));
	assert(w_csr != NULL);

    double * w_dense;
    posix_memalign((void**) &w_dense, ALN, (data_vocab_size*num_docs) * sizeof(double));
	assert(w_dense != NULL);
...
#pragma omp parallel for
	...
	while (iteration < max_iter) {
        ...
        SDDMM2(data_vocab_size, v_r, num_docs, K_transpose, u, w_csr, w_dense);
#if 0
